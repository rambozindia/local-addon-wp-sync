...
2. **Companion Plugin** (`blueburn-live-sync`) — Installed on your live WordPress site, exposes REST API endpoints for export/import
...
Upload the `companion-plugin/blueburn-live-sync` folder to your live site:

```bash
# Via WP-CLI
wp plugin install /path/to/blueburn-live-sync --activate

# Or manually
# Copy blueburn-live-sync/ to wp-content/plugins/ and activate in WP Admin
```
...
├── companion-plugin/
│   └── blueburn-live-sync/       # WordPress plugin for live site
│       ├── blueburn-live-sync.php # Plugin entry, temp-dir setup, PHP limit overrides
│       └── includes/
