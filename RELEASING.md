...
|----------|----------|---------|
| `blueburn-live-sync` (WordPress plugin) | Live WordPress sites | [WordPress.org Plugin Directory](https://wordpress.org/plugins/) |
...
- [ ] Run the official [Plugin Check (PCP)](https://wordpress.org/plugins/plugin-check/) plugin against `blueburn-live-sync` on a test site and fix anything it flags.
...
1. Build the ZIP:
   ```bash
   ./scripts/package-plugin.sh   # → dist/blueburn-live-sync-<version>.zip
   ```
...
4. The slug `blueburn-live-sync` must be unique on WordPress.org; the review team may ask you to rename if it's taken (search the directory first).
...
```bash
svn co https://plugins.svn.wordpress.org/blueburn-live-sync
cd blueburn-live-sync
# Copy plugin files into trunk/
cp -R /path/to/companion-plugin/blueburn-live-sync/* trunk/
svn add --force trunk
...
1. [ ] Bump version in `package.json` **and** `blueburn-live-sync.php` (header + `WPLSYNC_VERSION`) **and** `readme.txt` (`Stable tag:` + changelog).
